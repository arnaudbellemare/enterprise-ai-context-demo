...
async function evaluatePrompt(prompt: string, domain: string): Promise<any> {
  try {
    ...
    const sampleResponse = await generateSampleResponse(prompt, domain);
    
    // Evaluate with enhanced judge
    const evaluation = await judge.evaluate(prompt, sampleResponse);
    
    return {
      clarity: evaluation.dimensions.clarity,
      specificity: evaluation.dimensions.completeness,
      domain_relevance: evaluation.dimensions.relevance,
      completeness: evaluation.dimensions.completeness,
      score: evaluation.overall_score,
      reasoning_quality: evaluation.dimensions.reasoning_quality,
      accuracy: evaluation.dimensions.accuracy,
      factual_correctness: evaluation.dimensions.factual_correctness,
      coherence: evaluation.dimensions.coherence,
      depth: evaluation.dimensions.depth
    };
  } catch (error) {
    console.warn('Enhanced judge evaluation failed, using fallback');
    return await fallbackEvaluatePrompt(prompt, domain);
  }
}

/**
 * Generate sample response for evaluation
 */
async function generateSampleResponse(prompt: string, domain: string): Promise<string> {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Sample response generation failed: ${response.status}`);
    }

    const data = await response.json();
    return data.response || '';
  } catch (error) {
    console.warn('Sample response generation failed, using fallback');
    return `This is a sample response for the prompt: "${prompt}". The actual response would be generated by the AI model.`;
  }
}

/**
 * Fallback evaluation when enhanced judge fails
 */
async function fallbackEvaluatePrompt(prompt: string, domain: string): Promise<any> {
